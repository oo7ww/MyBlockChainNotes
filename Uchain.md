...
* 共识算法：POW、POS、DPOS、PBFT
  ...
  * POW
    ...
    工作证明是一个共识协议，认为网络中的有效区块链是计算量最大的链。这里提到的计算工作是为了将所有块添加到当前区块链而必须完成的工作。这项工作是由网     络节点完成的，必须在计算上有困难，以便使工作变得不简单，但也必须是可行的，以便在经过合理的努力之后可以实现。最终，网络将依靠提供该PoW的节点来维持     区块链。
